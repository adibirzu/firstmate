...
The marker-validation cases (annotated, blank, unterminated, and non-ASCII-blank first lines) and the `AGENTS.md`/`bin` shape case hold the plugin's predicate to the same verdicts as its shell owner `bin/fm-primary-scope-lib.sh`.
