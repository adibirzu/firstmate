...
OpenCode crewmate and scout launches set `OPENCODE_DISABLE_CLAUDE_CODE_PROMPT=1` and `OPENCODE_DISABLE_CLAUDE_CODE_SKILLS=1` and overlay `OPENCODE_CONFIG_CONTENT` with `permission.skill` deny except `no-mistakes`, so a 32K local slot is not filled by `~/.claude/CLAUDE.md` and the global skill catalog; secondmate OpenCode launches keep the previous permission-only overlay; the GB10 local LLM client contract in `Gb10LocalLlm.md` owns why that 32K slot cannot absorb the Claude Code compatibility catalog.
