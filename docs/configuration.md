...
Each new Herdr task tab is labelled with the captain-visible display name `fm-<host>-<project>-<task-id>`; the local, gitignored `config/herdr-session-host` supplies that host segment on a machine whose short hostname is not the recognizable token, and it is deliberately not inherited because the host is a property of the machine ([Session naming](herdr-backend.md#session-naming) owns the contract).
