...
The adapter locates the bottom-most recognized bordered row, a bare agent-glyph row (Claude `❯`, Codex `›`, cursor-agent `→` — the shared fleet-wide set in `bin/fm-composer-lib.sh`), or a Pi separator region admitted only when native identity is exactly Pi and state is idle, done, or blocked.
