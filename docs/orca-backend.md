...
A bare shell row is `unknown`, not an empty agent composer; only a bare agent-glyph row (`❯` `›` `→`, the shared fleet-wide set in `bin/fm-composer-lib.sh`) is recognized unbordered.
