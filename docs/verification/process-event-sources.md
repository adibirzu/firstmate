...
| argv integrity and parser-depth guard | an argument containing spaces survives as one argument, the runner passes a shell-looking non-parser argument literally, an unrepresentable newline is rejected at registration, and registration plus reconciliation refuse a recognized shell argv with eight nested command substitutions before Bash can parse it |
