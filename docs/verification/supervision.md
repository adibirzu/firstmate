...
When that binding misses on a lock this session still owns - Claude Code regenerates its session ID on `/clear` while the same process keeps the lock - ownership falls back to the shared harness-ancestry predicate, and a lock recorded to a different live session is outside that ancestry and still fails closed.
