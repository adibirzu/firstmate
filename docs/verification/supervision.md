...
Session-lock ownership in `bin/fm-session-lock-lib.sh` verifies a published identity binding rather than trusting Claude Code's reparented worker-pool ancestry.
...
`tests/fm-lock-ownership.test.sh` exercises new lock acquisition through `bin/fm-lock.sh`, proving same-session re-entry and sibling-session rejection after Claude worker-pool reparenting.
