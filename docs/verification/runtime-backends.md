...
Claude, Codex, OpenCode, Pi, pi-signed, Grok, Kimi, Muse, Cline, Cursor-agent, and Copilot share that backend cleanup boundary; their harness-specific hook files, tokens, and session-log sidecars are cleaned only after it, so no harness needs a separate endpoint parser.
