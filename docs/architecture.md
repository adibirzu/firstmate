...
A mate whose steering inbox holds no unhandled record and whose own pane is affirmatively parked at an empty prompt is healthy idle and is not escalated, because an un-drained row for a mate with nothing to drain cannot be a parent-actionable stall; anything short of that proven-idle state still escalates, so a genuine freeze stays visible.
...
For an ordinary crew whose current state is not authoritatively `working`, an explicit `paused:` or `captain-held` status declaration enters that cadence immediately, regardless of whether its agent is live, dead, or cannot be read.
The declaration does not suppress a genuinely busy worker: an authoritative `working` result still wins, so the ordinary busy and wedge paths remain intact.
The cadence is keyed to the declaration rather than the pane, so a clock or token counter that keeps an idle parked pane changing does not create bare stale wakes; the unchanged declaration re-surfaces once at the bounded interval.
An explicit `paused:` line naming `until <UTC ISO 8601>` is condition-aware: it is not rechecked before that time and is rechecked once as soon as the time passes, without extending the flat cadence, and a verified `captain-held` transfer is never rechecked while the away-posture record exists.
A secondmate's endpoint liveness is likewise never read; its status-declared wait takes the same bounded re-surface path, so a forgotten `paused:` or `captain-held` declaration cannot rot invisibly.
Its initial normal-mode status signal still surfaces through the no-verb path, while away mode self-handles that routine signal and owns the later recheck.
Fresh stale panes use the same current-state read before trusting the status log, so an active run or a proven busy worker outranks an old captain-relevant status-log line left behind before validation.
No-change heartbeats are also benign.
While nothing is in flight fleet-wide the interval between them doubles per consecutive no-change heartbeat up to `config/heartbeat-idle-seconds` (default 3600), and any task in flight holds the base cadence and resets the backoff, so a fresh spawn is never hidden behind an idle-fleet backoff.
...
`bin/fm-fleet-view.sh` renders that snapshot as Markdown for humans, plus the read-only per-host Herdr listing (schema `fm-fleet-herdr.v1`, unmatched sessions labeled unmanaged), while `bin/fm-bearings-snapshot.sh` provides the bounded bearings projection, so both views consume one structured contract instead of reparsing raw fleet files.
`bin/fm-fleet-pulse.sh` merges those two contracts into `fleet.json` plus a self-contained `fleet.html` served by the Pulse dashboard and republished on the supervision heartbeat and task-completion boundaries.
...
An invalid structured home never empties its record: every child the summary can still describe stays rendered — decisions, holds, queued and landed records, endpoint identities, counts, and provenance — while the specific invalidity kind and reason travel separately in `invalidity` and `current.reason` instead of replacing the data.
...
The daemon's declared-wait window ages against the crew's own pause-governing status line (`fm-classify-lib.sh`'s `status_paused_governing_line`, which folds the whole log rather than trusting the last line alone) rather than against pane busy state, because a declared wait can legitimately hold a pane busy, and only a later append whose verb genuinely supersedes the declaration - `working`, `done`, `blocked`, or `failed` - ends that routing and restores wedge detection; an intervening unrelated append, such as a `resolved:` line closing a different open decision, leaves the declaration standing.
...
The dispatch file is intentionally judgment-based: firstmate reads the natural-language rules at intake, chooses the best matching rule, resolves profile arrays itself from current quota output under the `AGENTS.md` section 4 intake boundary and the `router-dispatch` selection procedure, and passes only concrete `--harness`, `--model`, and `--effort` axes to `fm-spawn.sh`.
The mechanical selection, step-down chain, and machine-capacity verdict are owned by `llm-router-axi`, with `bin/fm-dispatch-select.mjs` kept as a thin forwarding shim and `bin/fm-capacity-lib.sh` as the admission wrapper.
Telemetry comes from `usage-axi`, the router's doctrine and thresholds live in `~/.config/llm-router-axi/policy.json`, both tools install from their GitHub main clones until npm publish, and `bin/fm-capacity.sh` is a thin wrapper over the router's capacity reading; [`docs/configuration.md`](configuration.md) owns those local surfaces and the install detail.
...
A `gh-axi pr merge` command that itself exits non-zero is recovered as a landed outcome, not a refusal, only when a best-effort live read taken before it proved `merged=false` and the confirmed read after it proves `merged=true`; an unreadable pre-state or a pre-state already merged keeps that non-zero exit fail-closed, and [`bin/fm-pr-merge.sh`](../bin/fm-pr-merge.sh)'s header owns the exact condition.
...
A pool worktree is only returned after teardown passes the slot-ownership proof: a contradictory task record or supported live endpoint refuses without touching either task, and no discard authority relaxes that - except a retirable pre-`spawn_gen` husk (no published incarnation, and its recorded endpoint absent or confirmed dead/agent-less), which the scan ignores so it can no longer pin a pool slot against the task actually being torn down.
