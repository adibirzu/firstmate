...
That selector runs only at dispatch; a model that depletes after dispatch is answered by the separate `modelFallback` chain walked through `bin/fm-runtime-handoff.sh` in place, whose schema and boundary are owned by [`docs/configuration.md`](configuration.md).
