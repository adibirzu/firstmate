...
The bounded bare shape supports verified agent glyphs such as Claude's borderless `❯`, Cursor's `→`, and Muse's `⟩`, with or without a trailing U+00A0 non-breaking space where the harness emits one, without relying on a cursor primitive that `read-screen` does not provide.
