...
| agy | `--model <id>` | `--effort <low\|medium\|high>` | Verified 2026-08-01 on Antigravity CLI 1.1.9. Base model ids require `--effort`; effort-baked model ids work alone; matching baked+effort works; conflict fails closed. Ceiling is `high`, and firstmate's shared `xhigh`/`max` tiers clamp down to `--effort high` rather than dropping the flag. Preferred form: base model + `--effort`. Crewmate launches only. |
...
| Model / effort interaction | Base model (e.g. `gemini-3.6-flash`) **requires** `--effort`. Baked suffix alone works. Matching baked + `--effort` works. Conflicting baked + `--effort` fails closed (`conflicts with --effort=...`). Preferred firstmate form: base model + `--effort`. Ceiling is `high`; firstmate clamps its shared `xhigh`/`max` tiers to `--effort high` rather than dropping the flag, because a base model id with no `--effort` refuses to launch and the spawn would only surface that as a trust-gate timeout. |
