...
The same placement-specific launch and locked bootstrap sweep also propagate the primary's declared inherited local material: `config/crew-dispatch.json`, `config/crew-harness`, `config/backlog-backend`, `config/backend`, `config/herdr-presentation-spaces`, `config/startup-memory-budget`, `config/spawn-capacity` (see docs/configuration.md "Machine capacity (config/spawn-capacity)"), and the one shared captain-preference file `data/captain-shared.md`.
