...
- `signal` or `stale` whose pause-governing status line (folded across the whole log, not just the last line) declares a wait, either a `paused:` external wait or a verified `captain-held` transfer, tracks the pause rather than a wedge whether its pane reads idle or busy.
  ...
  The window ages against that same pause-governing status line, so only a later append whose verb genuinely supersedes the declaration (`working`, `done`, `blocked`, or `failed`) ends this routing and restores wedge detection; an intervening unrelated append, such as a `resolved:` line closing a different open decision, leaves the declaration standing.
