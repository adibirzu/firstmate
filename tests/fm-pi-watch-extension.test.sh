...
test_opencode_watch_arm_coordinator_accepts_secondmate_linked_worktree() {
  local plugin base repo home log out status
  plugin="$ROOT/.opencode/plugins/fm-primary-watch-arm.js"
  base="$TMP_ROOT/opencode-sm-coordinator-base"
  repo="$TMP_ROOT/opencode-sm-coordinator-wt"
  home="$TMP_ROOT/opencode-sm-coordinator-home"
  log="$TMP_ROOT/opencode-sm-coordinator.log"
  fm_git_worktree "$base" "$repo" fm/opencode-sm-coordinator
  mkdir -p "$repo/bin" "$home/state" "$home/config"
  : > "$repo/AGENTS.md"
  : > "$home/state/task.meta"
  printf 'sm-coordinator-test\n' > "$repo/.fm-secondmate-home"
  cat > "$repo/bin/fm-watch-arm.sh" <<'SH'
#!/usr/bin/env bash
printf 'arm\n' >> "${FM_ARM_LOG:?}"
printf 'watcher: healthy pid=1 (beacon 0s)\n'
...
await new Promise((resolve) => setTimeout(resolve, 120));
if (status === "not-primary") {
  console.error(`expected armed/healthy, got ${status}`);
  process.exit(1);
}
if (!existsSync(process.env.FM_ARM_LOG)) {
  console.error("coordinator did not arm for secondmate linked worktree");
  process.exit(1);
}
EOF
)
  status=$?
  expect_code 0 "$status" "OpenCode watch coordinator must accept linked worktrees with .fm-secondmate-home marker"
  [ -z "$out" ] || fail "OpenCode coordinator secondmate-scope test printed output: $out"
  pass "OpenCode watcher coordinator accepts secondmate linked worktrees"
}
