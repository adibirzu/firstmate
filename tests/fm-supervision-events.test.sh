...
[ -e "$STATE_DIR/.herdr-escalated-$(fm_window_marker_key default:wG:pQ)" ] || fail "handle_push_transition must commit dedupe only after enqueue"
...
[ ! -e "$STATE_DIR/.herdr-escalated-$(fm_window_marker_key default:wG:pQ)" ] || fail "a failed durable enqueue must leave the blocked edge eligible for reconnect reconciliation"
