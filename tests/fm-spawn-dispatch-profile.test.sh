...
test_claude_threads_model_and_effort() {
  ...
  assert_contains "$launch" "CLAUDE_CODE_SEND_FEEDBACK=0" \
    "claude launch did not retain feedback safety control"
  assert_contains "$launch" "--strict-mcp-config --mcp-config '{\"mcpServers\":{}}'" \
    "claude launch did not retain strict empty MCP isolation"
  assert_contains "$launch" "--settings '{\"enabledPlugins\":{}}' --setting-sources project,local --no-chrome" \
    "claude launch did not retain plugin, user-settings, and browser isolation"
...
test_claude_forwards_firstmate_config_dir_when_set() {
  ...
  launch=$(cat "$LAUNCH_LOG")
  assert_contains "$launch" "CLAUDE_CONFIG_DIR='$CASE_DIR/claude-work' env -u CURSOR_AGENT -u CURSOR_INVOKED_AS -u GEMINI_CLI CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION=false CLAUDE_CODE_SEND_FEEDBACK=0 claude --dangerously-skip-permissions" \
