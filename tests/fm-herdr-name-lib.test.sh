...
#
# The display name is <prefix>-[<host>-]<project>-<task-id>, with the prefix
# from config/herdr-session-prefix (default adix) and the host segment present
# only when the home has an explicit host token. It is additive and never an
