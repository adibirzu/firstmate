...
# Fleet-wide verified defaults, shared by every adapter (the tmux path and the
# herdr/cmux/orca backends) so the idle-placeholder and bare-agent-glyph sets
# cannot drift per backend. Idle placeholders: grok "Type a message...", cline
...
# decomposes into the shared leading UTF-8 byte (0xE2) and spuriously matches ANY
# multibyte glyph in that range - including box-drawing corners like ╰,
# misclassifying a bordered composer's bottom border row as the bare shape.
# An alternation's branches are matched as whole literal byte sequences and stay
