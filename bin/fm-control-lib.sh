...
# Whether a harness's dead or missing agent-state reading may be trusted as
# proof that the previous agent is gone. A backend classifier can only report
# what it recognises, so an adapter it carries no liveness signature for reads
# absent rather than absent-because-exited. Adopting that reading would let a
# relaunch start a second agent into an endpoint that still holds a live one,
# so an adapter outside this set is refused instead of trusted.
