...
# open keyed status decision, including when an earlier inventory already exists.
# A decision key is a durable identity: once `complete` has closed it with a
# `captain-held [key=...]` transfer, that key is spent, and a later needs-decision
# or blocked event carrying the same key is a different call the existing hold
# cannot represent, so completion refuses it. Untagged events all carry `default`,
# so a second untagged call must be reopened under an unused key. With a non-empty
# inventory, every listed task is verified durable (actively captain-held, or
# closed with a recorded answer), the inventory is unioned idempotently into the
# metadata, and every still-open keyed status decision is transferred to its
# durable owner with a `captain-held [key=...]` status close naming the inventory.
# Later review passes may add ids. A post-teardown visual review can complete
# against the surviving report and tasks without recreating task state.
...
# An open status key may only be inventoried against a captain hold that
# represents THAT call. Refuse a key whose durable identity an earlier
# captain-held transfer already spent, because the existing hold answers the
# earlier call under that key, not the one open now.
...
command_verify() {
  local origin=${1:-} meta status_file reviewed keys entry key open resolved
  [ "$#" -eq 1 ] || { usage >&2; exit 2; }
  validate_slug origin-id "$origin"
  meta="$STATE/$origin.meta"
  status_file="$STATE/$origin.status"
