...
3. Return to a clean default-branch base, prove it is current, then create your branch: run \`"$FM_ROOT/bin/fm-base-check.sh" .\` and follow what it prints, and only when it reports current (exit 0): \`git checkout -b fm/$ID\`.
