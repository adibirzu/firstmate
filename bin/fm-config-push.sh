...
while IFS='|' read -r id home _window meta; do
  ...
  if [ -n "$remote_host" ]; then
    ...
    if ! fm_secondmate_nudge_write "$STATE" "$id" "$home" "$remote_generation" remote \
    ...
    if remote_out=$(FM_CONFIG_INHERIT_LIVE=1 \
      ...
      if [ "$remote_nudge" -eq 1 ]; then
        if ! remote_delivery_id=$(fm_secondmate_nudge_delivery_id "$id" "$remote_generation"); then
          echo "  config-reread: cannot derive a delivery id"
          errors=1
          fm_lock_release "$remote_lock" || true
          continue
        fi
        if FM_HOME="$FM_HOME" FM_ROOT_OVERRIDE="$FM_ROOT" FM_STATE_OVERRIDE="$STATE" \
          ...
          --fire-and-forget "$remote_delivery_id" \
          "$FM_REMOTE_SECOND_MATE_NUDGE_MESSAGE" >/dev/null 2>&1; then
          rm -f -- "$remote_marker"
