...
4. Report status by appending one line:
   `echo "{state}: {one short line}" >> '/tmp/fm-graph-evidence.d6lswm/state/evidence-ship.status'`
...
# Project memory
If `AGENTS.md` or `CLAUDE.md` already exists, or if this task produced durable project-intrinsic knowledge, run `/Users/adrianb/.no-mistakes/worktrees/80e4cf1781af/01M05KAJQYJDZAP2KP064ADKC4/bin/fm-ensure-agents-md.sh .` in the worktree.
Record only project knowledge useful to almost every future session.
For anything the codebase already shows, prefer a pointer to the authoritative file, command, or doc over copying the detail.
If you touch a project `AGENTS.md` that lacks `## Maintaining this file`, add that short self-governance section from `/Users/adrianb/.no-mistakes/worktrees/80e4cf1781af/01M05KAJQYJDZAP2KP064ADKC4/bin/fm-ensure-agents-md.sh` in the same pass.
