...
The verified harnesses are `claude`, `codex`, `opencode`, `pi`, `pi-signed`, `grok`, `kimi`, `cursor-agent`, and `cursor`, plus `muse`, `gemini`, `agy`, `cline`, and `copilot` for crewmate and scout launches only; never dispatch on an unverified adapter, and never select one of those five for a secondmate (`docs/configuration.md` "Harness support" owns the per-kind verified set).
