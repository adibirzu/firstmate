...
When a worker's model depletes mid-run, switch models within the same harness automatically instead of blocking, parking, or escalating a routine depletion; `bin/fm-model-fallback.sh <task-id> apply` owns that response mechanically - evidence classification, chain selection, and the guarded in-place relaunch through `bin/fm-runtime-handoff.sh` - so invoke it rather than improvising the steps.
...
Every worker status passes through the shared depletion classifier, which accepts framed 429s, spending-limit 403s, explicit rate-limit or `RESOURCE_EXHAUSTED` errors, and named quota or credit exhaustion but rejects bare `limit` and unframed 429 text; telemetry-backed providers additionally record the verified failure through `record-failure`.
