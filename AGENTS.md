...
The verified harnesses are `claude`, `codex`, `opencode`, `pi`, `pi-signed`, `grok`, `kimi`, `cline`, `cursor-agent`, and `copilot`, plus `muse` for crewmates and scouts only; never dispatch on an unverified adapter.
